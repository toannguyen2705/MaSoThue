const scrapeCategory = async (browser, url) =>
  new Promise(async (resolve, reject) => {
    try {
      ...
      await page.goto(url);
      await page.waitForSelector("form-control search-field tax-search-input");
      await page.focus("form-control search-field tax-search-input");
      // await page.$eval(
      //   "form-control search-field tax-search-input",
      //   (el) => (el.value = "kế toán")
      // );
      await page.type("tax-search-input", "kế toán");
      await page.click(
      ...
      for (let k = 1; k <= 2; k++) {
        ...
        for (let j = 2; j <= taxListing.length * 2; j = j + 2) {
          ...
          await page.goto(url.concat(`&page=${k}`));
        }
      }
      resolve();
    } catch (error) {
